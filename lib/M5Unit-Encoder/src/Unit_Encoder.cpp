...
bool Unit_Encoder::begin(TwoWire *wire, uint8_t addr, uint8_t sda, uint8_t scl,
                         uint32_t speed) {
    _wire  = wire;
    _addr  = addr;
    _sda   = sda;
...
int32_t Unit_Encoder::getEncoderValue(void) {
    int32_t value = 0;
...
/*! @brief Read the encoder inc value.
    @return The value of the encoder that was read */
int32_t Unit_Encoder::getIncEncoderValue(void) {
    int32_t value = 0;
...
bool Unit_Encoder::getButtonStatus(void) {
    uint8_t data;
...
void Unit_Encoder::setLEDColor(uint32_t color, uint8_t index) {
    uint8_t data[4];
    data[3] = color & 0xff;
    data[2] = (color >> 8) & 0xff;
...
uint32_t Unit_Encoder::getLEDColor(void) {
    uint8_t data[4];
    uint32_t value = 0;
...
void Unit_Encoder::setEncoderValue(int32_t encoder) {
    writeBytes(_addr, ENCODER_REG, (uint8_t *)&encoder, 4);
}

void Unit_Encoder::resetEncoder(void) {
    uint8_t data = 1;
...
bool Unit_Encoder::getDevStatus(void) {
    _wire->beginTransmission(_addr);
...
uint8_t Unit_Encoder::getBootloaderVersion(void) {
    _wire->beginTransmission(_addr);
...
uint8_t Unit_Encoder::getFirmwareVersion(void) {
    _wire->beginTransmission(_addr);
...
uint8_t Unit_Encoder::setI2CAddress(uint8_t addr) {
  uint8_t temp[2] = {0};
...
uint8_t Unit_Encoder::getI2CAddress(void) {
  uint8_t temp[2] = {0};
...
void Unit_Encoder::jumpBootloader(void) {
    uint8_t value = 1;
